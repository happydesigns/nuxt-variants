...
const { data: post } = await useAsyncData('hello-world', () =>
  ...
  <div :style="styles.page">
    ...
    <div :style="styles.content">
      ...
      <section :style="styles.section">
        ...
        <p :style="styles.desc">
          <code>useVariant('article')</code> deeply merges the registry and <code>app.config.ts</code>
          override at runtime. The <code>app.config.ts</code> sets <code>authorBox: false</code>,
          overriding the <code>nuxt.config.ts</code> default of <code>true</code>.
          <br>
          <code>useVariantExtends('article', 'seo')</code> returns
          <strong>{{ extendsSeo }}</strong> — proving the inheritance graph is also
          available reactively at runtime.
        </p>

        <div :style="styles.card">
          <div :style="styles.cardHeader">
            <span :style="styles.cardTitle">useVariant('article')</span>
            <span :style="{ ...styles.pill, background: '#d1fae5', color: '#065f46' }">reactive</span>
          </div>
          <table :style="styles.table">
            <tr v-for="(val, key) in articleVariant" :key="key">
              <td :style="styles.key"><code>{{ key }}</code></td>
...
const styles = {
  ...
  cardHeader: { display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 20px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0' },
  cardTitle: { fontFamily: 'monospace', fontSize: '13px', fontWeight: '700', color: '#1e293b' },
  pill: { fontSize: '11px', padding: '2px 8px', background: '#e0e7ff', color: '#3730a3', borderRadius: '10px', fontWeight: '600' },
  table: { width: '100%', borderCollapse: 'collapse' as const, fontSize: '13px' },
  key: { padding: '10px 20px', color: '#64748b', fontFamily: 'monospace', width: '160px', borderBottom: '1px solid #f1f5f9' },
  value: { padding: '10px 16px', color: '#0f172a', fontWeight: '600', fontFamily: 'monospace', borderBottom: '1px solid #f1f5f9' },
  source: { padding: '10px 20px', color: '#94a3b8', fontSize: '12px', borderBottom: '1px solid #f1f5f9' },
  override: { padding: '1px 7px', background: '#fef3c7', color: '#92400e', borderRadius: '6px', fontWeight: '600' },
}
</script>
