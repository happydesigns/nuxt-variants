...
Use `@happydesigns/nuxt-variants/schemas` in `content.config.ts`. The
`#variants-schemas` virtual module is generated for Nuxt/Vite runtime imports,
but `content.config.ts` can execute before Vite aliases are fully available.
