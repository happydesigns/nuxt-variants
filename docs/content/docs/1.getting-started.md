...
Values in `app.config.ts` win over values from `nuxt.config.ts`. Runtime config
may only override `config` and `active` for names already registered in
`nuxt.config.ts`. Keep names and `extends` in the build-time registry so
runtime behavior, generated types, and Content schemas share one graph.
